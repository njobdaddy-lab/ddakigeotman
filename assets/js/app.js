document.write('<script src="./assets/js/app-core.js?v=tabs5"><\/script>');
...
(function setupThreeFreeToolTabs(){
  ...
  try{
    if(!PRESET_PROMPT_CARDS.some(x=>x.id==='sticker-photo')){
      PRESET_PROMPT_CARDS.splice(3,0,{id:'sticker-photo',category:'fun',categoryLabel:'재미 변환',module:'create',title:'스티커 사진',desc:'포토부스 감성',prompt:NMD_PRESETS['sticker-photo'].shortPrompt});
    }
  }catch(e){}
    ...
    styleModule.innerHTML=`
      <div class="module-head">
        ...
        <div>
          ...
          <div class="module-sub">수채화·웹툰 같은 화풍부터 요즘 많이 쓰는 재미있는 변환까지 골라보세요.</div>
        </div>
      </div>
  ...
  if(gallery&&styleCard&&!styleCard.contains(gallery))styleCard.appendChild(gallery);
  createModule.querySelectorAll('.preset-gallery-field').forEach(el=>{if(!styleCard.contains(el))el.remove()});
  ...
  if(gallery){
    ...
    gallery.querySelectorAll('[data-preset-filter]').forEach(btn=>{
      if(btn.dataset.presetFilter==='practical')btn.style.display='none';
  ...
  function syncPresetFilterButtons(value){document.querySelectorAll('#module-style [data-preset-filter]').forEach(btn=>btn.classList.toggle('active',btn.dataset.presetFilter===value))}
  ...
  function replaceToolScreen(name,pushFromHome=false){
    const screen='module-'+name;
    if(name!=='style'){
      if(pushFromHome&&typeof openModule==='function')return openModule(name);
      if(typeof switchFreeTool==='function')return switchFreeTool(name);
      if(typeof renderScreen==='function')return renderScreen(screen);
      return;
    }
    if(pushFromHome){
      if(typeof pushScreen==='function')return pushScreen(screen);
      try{history.pushState({nmdScreen:screen},'',location.href.split('#')[0])}catch(e){}
      if(typeof renderScreen==='function')return renderScreen(screen);
      return;
    }
    try{
      if(typeof fallbackStack!=='undefined'){
        if(fallbackStack.length)fallbackStack[fallbackStack.length-1]=screen;else fallbackStack.push(screen);
      }
      history.replaceState({nmdScreen:screen},'',location.href.split('#')[0]);
    }catch(e){}
    if(typeof renderScreen==='function')renderScreen(screen);
  }
  window.openStyleTool=function(){replaceToolScreen('style',typeof currentScreen!=='undefined'&&currentScreen==='home')};
  ...
  window.useStylePromptInCreate=function(){
    ...
    replaceToolScreen('create',false);
    setTimeout(()=>{
  ...
  [['module-create','create'],['module-photo','photo'],['module-style','style']].forEach(([id,active])=>{
    ...
    tabDefs.forEach(([name,label])=>{
      const btn=document.createElement('button');btn.type='button';btn.textContent=label;btn.classList.toggle('active',name===active);
      btn.addEventListener('click',()=>{
        replaceToolScreen(name,false);
        if(name==='style')setTimeout(()=>{syncPresetFilterButtons(presetFilter==='style'||presetFilter==='fun'?presetFilter:'all');renderStylePresetCards()},20);
      });
      nav.appendChild(btn);
    });
  ...
  const originalRenderPresetCards=window.renderPresetCards;
  function renderStylePresetCards(){
    const grid=document.getElementById('presetGrid'),more=document.getElementById('presetMoreBtn');
    if(!grid||!more)return;
    ...
    visible.forEach(card=>{
      const btn=document.createElement('button');btn.type='button';btn.className='preset-card';
      btn.innerHTML=`<span class="preset-category">${card.categoryLabel}</span><strong>${card.title}</strong><small>${card.desc}</small>`;
      btn.addEventListener('click',()=>window.applyFunPromptCard(card.id,false));grid.appendChild(btn);
    });
    more.style.display=filtered.length>8?'block':'none';more.textContent=presetExpanded?'접기':'예시 더 보기';
  }
  try{renderPresetCards=renderStylePresetCards}catch(e){window.renderPresetCards=renderStylePresetCards}

  window.applyFunPromptCard=function(id,useFull=false){
    const preset=getPreset(id);if(!preset)return;
    if(preset.category==='edit'){
      replaceToolScreen('photo',typeof currentScreen!=='undefined'&&currentScreen==='home');
    ...
    replaceToolScreen('style',typeof currentScreen!=='undefined'&&currentScreen==='home');
    setTimeout(()=>{
      const base=getBaseCard(id);presetFilter=base?.category==='style'?'style':'fun';presetExpanded=false;syncPresetFilterButtons(presetFilter);renderStylePresetCards();fillStylePrompt(id,useFull,true);
  ...
  window.openPresetGalleryHome=function(){
    presetFilter='all';presetExpanded=false;replaceToolScreen('style',typeof currentScreen!=='undefined'&&currentScreen==='home');
    setTimeout(()=>{syncPresetFilterButtons('all');renderStylePresetCards();document.querySelector('#module-style .preset-gallery-field')?.scrollIntoView({behavior:'smooth',block:'start'})},60);
  };

  document.querySelectorAll('#module-style [data-preset-filter]').forEach(btn=>{
    btn.replaceWith(btn.cloneNode(true));
  });
  document.querySelectorAll('#module-style [data-preset-filter]').forEach(btn=>btn.addEventListener('click',()=>{
    presetFilter=btn.dataset.presetFilter||'all';presetExpanded=false;syncPresetFilterButtons(presetFilter);renderStylePresetCards();
  }));
